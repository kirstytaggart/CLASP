...
Edited
======
WARNING - USES TABS NOT SPACES FOR INDENTATION!

J.Lyman     Incorporates bpm directly and adjusted verbosity (06/2011)
            Added logger to fit subtraction pipe (10/2011)
            Added np.Inf border if using trim around self.mask (03/2012)
            Will not bother entering lacosmic iteration if maxiter=0, but will
            still clean bpm if supplied (03/2012)
...
logger = logging.getLogger("run-subpipe.subpipe.cosmics")
...
class cosmicsimage:
	...
	def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0, sigclip = 5.0, sigfrac = 0.3, objlim = 5.0, satlevel = 50000.0, bpm=None, trimborder=0, usebin=False, verbose=False):
		"""
		...
		"""
		logger.debug("setting up cosmicsimage")
		...
		if bpm:		
			bpmarray, bpmheader = fromfits(bpm,verbose = self.verbose)
			if bpmarray.shape != rawarray.shape:
				logger.warning("image (%s) and bad pixel mask (%s) shapes are "
							   "not equal! disregarding bpm for this image."
							   % (rawarray.shape,bpmarray.shape))
				self.mask = np.cast['bool'](np.zeros(self.rawarray.shape))
			else:
				self.mask = np.cast['bool'](bpmarray)
		else:
	...
	def clean(self, mask = None, verbose = None):
		...
		# A counter to prevent an incorrect BPM from trying to make use interpolate over huge areas
		numhuge = 0
		# A loop through every cosmic pixel :
		for cosmicpos in cosmicindices:
			if numhuge > 9:
				logger.error("check your bad pixel mask, "
								"attempting to clean too many large pixel clumps")
				sys.exit(3)
			x = cosmicpos[0]
			y = cosmicpos[1]
			...
			if np.alen(goodcutout) >= 25 :
				...
				logger.warning("found a large clump of pixels to correct")
				numhuge +=1
	...
	def findsatstars(self, verbose = None):
		...
		logger.info("%i saturated stars found" % nsat)
	...
	def lacosmiciteration(self, verbose = None):
        ...
        # Remove any cosmics in the border that will be trimmed as unecessary to clean.
		if self.trimborder:
			finalsel[:self.trimborder] = False
			finalsel[-self.trimborder:] = False
			finalsel[:,:self.trimborder] = False
			finalsel[:,-self.trimborder:] = False
	...
	def run(self, maxiter = 4, verbose = False):
		...
		if maxiter == 0 and self.bpm:
			logger.info("cleaning the bad pixel mask")
			self.clean(verbose=verbose)
			return

		if maxiter == 0 and not self.bpm:
			logger.warning("cleaning called with no iterations or bpm, skipping")
			return
		...
		logger.info("beginning %i L.A. cosmic iterations" % maxiter)
...
def tofits(outfilename, pixelarray, hdr = None, verbose = False):
	"""
	...
	if os.path.isfile(outfilename):
		logger.debug("removing old %s" % os.path.basename(outfilename))
	...
	logger.debug("wrote %s" % os.path.basename(outfilename))
