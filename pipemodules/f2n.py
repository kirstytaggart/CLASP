...
Edited 01/2013 - JDL:
    - When calculating scale, remove zero-value pixels, input by 
      wregister/xyxymatchfrom, from the array to help with images that 
      have been heavily aligned
...
f2n.py is a python module to make a well-scaled PNG file out of a FITS image.
...
class f2nimage:
	def __init__(self,numpyarray=None, shape = (100,100), fill = 10000.0, verbose=False):
		"""
...
#		return pythoncopy.deepcopy(self)	
	...
	def setzscale(self, z1="auto", z2="auto", nsig=3, samplesizelimit = 10000, border=300):
		...
		# JDL edit: remove zero-value pixels
		nozerocalcarray = calcarray[calcarray!=0]
		# ...but only if we have enough pixels left!
		if nozerocalcarray.size > calcarray.size/10.:
			calcarray = nozerocalcarray		
		...
		if z1 == "auto" or z2 == "auto" or z1 == "flat" or z2 == "flat":
			...
			if z2 == "auto" :
				...
				n = round(0.99 * statsel.size) #0.9995
	...
	def drawcircle(self, x, y, r = 10, colour = None, label = None):
		...
		if label != None:
			...
			self.draw.text((pilx - float(textwidth)/2.0 + 1, pily + pilr + 2), label, fill = colour, font = self.labelfont)
...
# 		
	...
	def drawstarsfile(self, filename, r = 10, colour = None):
		...
		for i, line in enumerate(lines):
			...
			name = elements[0] #TODO change to -1 to read in coo files?
			x = float(elements[1])
			y = float(elements[2])
...
def fromfits(infile, hdu = 0, verbose = False):
	"""
...
def rebin(a, newshape):
	"""
	Auxiliary function to rebin ndarray data. Into a smaller ndarray of the same rank whose dimensions
        are factors of the original dimensions. An array with 6 columns and 4 rows
